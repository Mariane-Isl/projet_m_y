<?php
session_start();
$page_title = "Rapport 001 — Chiffre d'Affaire Fournisseur";
...
<style>
    ...
    .filter-ctrl:focus {
        ...
        box-shadow: 0 0 0 2px rgba(59, 158, 255, .12);
    }
    ...
    .total-pill.green .total-pill-icon {
    ...
    #tableRapport tbody tr:hover td {
    ...
    .dataTables_wrapper .dataTables_filter input:focus {
        border-color: var(--accent-blue) !important;
        box-shadow: 0 0 0 2px rgba(59, 158, 255, .12) !important;
    }
        ...
        <div class="results-card">
            <div class="results-header">
                ...
                <div style="display:flex;align-items:center;gap:10px;">
                    <span class="results-count" id="results-count">0 facture(s)</span>
            ...
            <div style="padding:16px 18px;overflow-x:auto;">
                <table id="tableRapport" class="table w-100 mb-0">
                    <thead>
                        <tr>
                            <th>N° Facture</th>
                            <th>Date Facture</th>
                            ...
                            <th>Statut OV</th>
                            <th>Date Statut OV</th>
                            <th>Structure</th>
                            <th>Gestionnaire</th>
                            <th>Action</th> <!-- Changement ici -->
                        </tr>
                    </thead>
                    <tbody id="table-body">
                        <tr>
                            <td colspan="14" style="text-align:center;color:var(--text-muted);padding:30px!important;">
...
<script>
    $(document).ready(function() {
        ...
        // ── Badge helper ──────────────────────────────────────────────────────
        function sfBadge(code, label) {
            const map = {
                RECU: 'sf-recu',
                PAYE: 'sf-paye',
                REJETER: 'sf-rejeter',
                AFFECTED: 'sf-affected'
            };
            const cls = map[code] || 'sf-default';
            return label ? `<span class="badge-sf ${cls}">${label}</span>` : '<span style="color:var(--text-muted)">—</span>';
        }
        ...
        function fmtMontant(n) {
            return parseFloat(n).toLocaleString('fr-DZ', {
        ...
        function doSearch() {
            ...
            fetch(API, {
                ...
                .then(res => {
                    ...
                    res.data.forEach(r => {
                        html += `<tr>
                        <td><strong style="color:var(--accent-blue);font-size:.8rem;">${r.Num_facture}</strong></td>
                        <td>${fmtDate(r.date_facture)}</td>
                        <td><strong>${fmtMontant(r.Montant)}</strong></td>
                        ...
                        <td>${sfBadge(r.statut_code, r.statut_facture)}</td>
                        <td style="font-size:.76rem;color:var(--text-muted);">${fmtDate(r.date_statut)}</td>
                        <td><span style="font-size:.76rem;">${r.Num_OV || '—'}</span></td>
                        <td>${ovBadge(r.statut_ov_code, r.statut_ov)}</td>
                        <td style="font-size:.76rem;color:var(--text-muted);">${fmtDate(r.date_statut_ov)}</td>
                        <td>${r.structure || '—'}</td>
                        <td>${r.gestionnaire ? r.gestionnaire.trim() : '—'}</td>
                    ...
                    if (!html) {
                        html = `<tr><td colspan="14" style="text-align:center;color:var(--text-muted);padding:30px!important;">Aucun résultat trouvé.</td></tr>`;
                    }

                    document.getElementById('table-body').innerHTML = html;
                    document.getElementById('results-count').textContent = res.count + ' facture(s)';
                    document.getElementById('btn_excel').style.display = res.count > 0 ? 'inline-flex' : 'none';

                    if (res.count > 0) {
                        dtInstance = $('#tableRapport').DataTable({
                            pageLength: 20,
                            ...
                            columnDefs: [{
                                orderable: false,
                                targets: [13] // Désactive le tri sur la colonne Action (index 13)
                            }]
                        });
                    }
                    ...
                    Swal.fire({
                        ...
                        text: res.count + ' facture(s) trouvée(s).',
                        timer: 1800,
        ...
        $('#btn_reset').on('click', function() {
            ...
            document.getElementById('table-body').innerHTML = `<tr><td colspan="14" style="text-align:center;color:var(--text-muted);padding:30px!important;">Sélectionnez vos critères et cliquez sur Rechercher.</td></tr>`;
            document.getElementById('results-count').textContent = '0 facture(s)';
        ...
        $('#btn_excel').on('click', function() {
            ...
            link.download = 'rapport_fournisseur_' + new Date().toISOString().slice(0, 10) + '.csv';
            link.click();
        });
